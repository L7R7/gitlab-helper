...
import Gitlab.Client.MTL (ApiToken (ApiToken), BaseUrl (BaseUrl), UserAgent (UserAgent))
...
partialConfigParser =
  btraverse maybeFirstParser
    $ PartialConfig
      ...
      (UserAgent <$> E.var (E.str <=< E.nonempty) "GLH_USER_AGENT" (E.help "User-Agent to use for requests against the Gitlab API."))
